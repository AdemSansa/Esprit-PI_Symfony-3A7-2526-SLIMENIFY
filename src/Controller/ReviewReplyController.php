...
#[Route('/api/review-replies', name: 'api_review_replies_')]
class ReviewReplyController extends AbstractController
{
    public function __construct(
        private ReviewReplyRepository $repository,
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $review = $this->reviewRepo->find($data['review_id']);
        if (!$review) return $this->json(['error' => 'Review not found'], Response::HTTP_BAD_REQUEST);

        $r = new ReviewReply();
        $r->setContent($data['content']);
        $r->setReview($review);
        if (isset($data['therapist_id'])) {
            $therapist = $this->userRepo->find($data['therapist_id']);
            $r->setTherapist($therapist);
        }
    ...
    private function serialize(ReviewReply $r): array
    {
        return [
            'id'           => $r->getId(),
            'content'      => $r->getContent(),
            'review_id'    => $r->getReview()->getId(),
            'therapist_id' => $r->getTherapist()?->getId(),
